...
> **문서명/역할**: 프로젝트 운영 수칙 — 일하는 방식, 원칙, 보안, 완료 기준  
> **버전**: `v1.1`  
> **최종 갱신일**: 2026-08-31  
> **작업 담당 AI**: Claude (Anthropic)  
> **사용 모델**: Claude Opus 5  
> **문서 상태**: 최종 승인
...
점수를 "평균 낸 숫자"로 다루면 치명 결함이 묻히고, 재보지 않은 항목이
통과처럼 보인다. 아래는 그 두 가지를 막는 하드 게이트다.

- 치명 결함은 총점 평균에 흡수시키지 않는다. Gate 조건에 걸린 항목은
  하위 점수가 아무리 높아도 해당 Domain 등급을 제한한다.
  (예: `noindex` 페이지는 콘텐츠 점수와 무관하게 SEO 등급 상한을 적용)
- Coverage가 기준 미만이면 확정 점수를 표기하지 않고 잠정 결과로 표시한다.
  낮은 Coverage를 숨기려고 점수를 반올림하거나 미측정을 제외해 비율을
  끌어올리지 않는다.
- 페이지 단위 결과를 사이트 단위 결론으로 일반화하지 않는다. 사이트 점수는
  별도 크롤링 근거를 갖춘 경우에만 산출한다.
- 신규 평가 항목은 측정가능성 감사(A/B/C/D 분류)를 통과하기 전에 ruleset에
  넣지 않는다. C·D 등급은 필요한 인프라가 준비된 뒤에만 승격한다.
